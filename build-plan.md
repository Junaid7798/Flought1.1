...
## PHASE 6.5 — STORE SUBMISSIONS (1 week parallel to V1 polish)

**6.5.1 — App icons**
```bash
npm install --save-dev @capacitor/assets
# Add master icon 1024x1024 to resources/icon.png
npx capacitor-assets generate
```

**6.5.2 — Android (Google Play — $25 one-time)**
```bash
npx cap add android
npx cap sync
# Open Android Studio → Build → Generate Signed AAB
# Submit to Google Play Console internal testing first
```

**6.5.3 — iOS (App Store — $99/year Apple Developer)**
```bash
npx cap add ios
npx cap sync
# Open Xcode → configure signing → submit to TestFlight first
# Privacy Policy URL required before App Store submission
```

**6.5.4 — Windows (Microsoft Store — $19 one-time)**
```bash
npm run tauri build -- --target x86_64-pc-windows-msvc
# Package as MSIX, submit to Microsoft Partner Center
```

**6.5.5 — Mac (same $99 Apple account)**
```bash
npm run tauri build
# Notarise via xcrun notarytool
# Submit to Mac App Store or distribute as direct download
```

---

## POST-LAUNCH — BEFORE TELLING ANYONE

Manual test checklist:
- [ ] Fresh browser profile — run full onboarding end to end
- [ ] Create 10 thoughts, link them, verify graph shows edges
- [ ] Disconnect internet — verify app still opens and works
- [ ] Reconnect — verify sync status updates
- [ ] Test on real iPhone Safari and Android Chrome
- [ ] `npm run build` — zero errors
- [ ] .env.local is NOT in git: `git log --all -- .env.local` returns nothing
