<script lang="ts">
	...
	let edgeSub: { unsubscribe: () => void } | null = null; // subscription reference for cleanup
	...
	onMount(async () => {
		...
		edgeSub = getEdgesByLibrary(libraryId).subscribe({
			next: (edges: Edge[]) => {
			...
			error: (err) => {
				handleError(err, 'SerendipityCollider.subscription', true);
			}
		});
	});
